 ...
 *
 * <p>
 * The CXF module assembles a CXF-based server-side application for hosting the
 * SOAP endpoints.</p>
 *
 * <p>
 * Note that the CXF module is disabled by default, so you must enable it in the
 * enunciate configuration file, e.g.:</p>
 *
 * <code class="console">
 * &lt;enunciate&gt; &nbsp;&nbsp;&lt;modules&gt;
 ...
 *
 * <p>
 * You should also be aware that the CXF module is not, by default, on the
 * classpath when invoking Enunciate. For more information, see <a
 * href="http://docs.codehaus.org/display/ENUNCIATE/Using+CXF+or+XFire">using
 * CXF or XFire</a>.</p>
 *
 * <ul>
 ...
 *
 * <p>
 * The "generate" step generates the spring configuration file.</p>
 *
 * <h1><a name="config">Configuration</a></h1>
 *
 * <p>
 * The CXF module supports the following configuration attributes:</p>
 *
 * <ul>
 * <li>The "validate" attribute (boolean) can be used to turn off CXF validation
 * errors. Default: true</li>
 * <li>The "enableJaxws" attribute (boolean) can be used to disable the JAX-WS
 * support, leaving the JAX-WS support to another module if necessary. Default:
 * true</li>
 * <li>The "enableJaxrs" attribute (boolean) can be used to disable the JAX-RS
 * support, leaving the JAX-RS support to another module if necessary. Default:
 * true</li>
 * <li>The "useSubcontext" attribute is used to enable/disable mounting the
 * JAX-RS resources at the rest subcontext. Default: "true".</li>
 * <li>The "useWsdlRedirectFilter" attribute is used to disable the use of the
 * Enunciate-provided wsdl redirect filter that handles the requests to
 * ?wsdl</li>
 * <li>The "jaxwsServletTransform" attribute is used to specify an XSLT
 * transform file that will be used for additional customization of the
 * Enunciate-generated cxf-jaxws-servlet.xml file. Default: none.</li>
 * <li>The "jaxrsServletTransform" attribute is used to specify an XSLT
 * transform file that will be used for additional customization of the
 * Enunciate-generated cxf-jaxrs-servlet.xml file. Default: none.</li>
 * </ul>
 *
 * <p>
 * The CXF module also supports a list of <tt>jaxws-property</tt> child elements
 * that each support a 'name' and 'value' attribute. This can be used to
 * configure the CXF JAX-WS mechanism, and the properties will be passed along
 * to the cxf configuration.</p>
 *
 * <h1><a name="artifacts">Artifacts</a></h1>
 *
 * <p>
 * The CXF deployment module exports no artifacts.</p>
 *
 * @author Ryan Heaton
            ...
            if (enableJaxrs) {
                ...
                for (RootResource resource : model.getRootResources()) {
                    for (ResourceMethod resourceMethod : resource.getResourceMethods(true)) {
                        ...
                        for (ReferenceType r : resourceMethod.getThrownTypes()) {  
                            try {
                                Class clazz = Class.forName(r.toString());
                                info("%s", clazz.getCanonicalName());
                                if(clazz.isAssignableFrom(WebApplicationException.class)){                               
                                 info("WebapplicationException found !!");
                                } 
                            // info("%s throws: %s - %s", resourceMethod.getFullpath(), r, resourceMethod.getStatusCodes());
                            } catch (ClassNotFoundException cnf) {
                                warn("class not found: %s" , r.toString() );
                            }
